...
import { planned_designs } from './event-planning-jobs'



const AllJobsPlanned = () => {
    const page_item_number = 9
     const [state, setstate] = useState({
         filtered:planned_designs,
         page_data: planned_designs.slice(0,page_item_number)
     })
...
const search = e=>{
    ...
    setstate({
        ...state,
        filtered: planned_designs.filter(item=>e.target.value.toLowerCase().includes(
            item.name.toLowerCase()
        )||
        searchthroughArray(item.speciality,e.target.value)
        ),
        page_data:planned_designs.filter(item=>item.name.toLowerCase().includes( e.target.value.toLowerCase())
