...
const ApproveJobsPart = ({jobsData}) => {

    return (
        <div>
            {console.log(jobsData.acceptedjobs.length)}
            <Card className = "my-5" title = "Applied Jobs">
                <Skeleton loading = {jobsData.getPendingLoading} >
                   
                <Row>
                {
                        jobsData.acceptedjobs.length >= 0?
                        <>
                        <Empty>
                            <p>You have no approved job</p>
                        </Empty>
                        </>:
                        jobsData.acceptedjobs.map(job=>(
...
export default connect(mapStateToProps) (ApproveJobsPart)
