import { Button, Card, Input, InputNumber, Select ,Form, message, Result, notification} from 'antd'
...
// toglenLoading()
    ...
    const handleCreateRecipient =()=>{
        ...
        if (token === null){
            ...
            const fd  = new FormData()
            fd.append("bank_code",select_value.bank_code)
            fd.append("name",select_value.name)
            fd.append("account_number",select_value.account_number)
            
            axios.post(createPaymentRecpeintPath,fd,config).then(res=>{
                message.success("Continue to cashout")
                setState({
                    ...state,
                    cur_form_page:state.cur_form_page + 1,
                    loading:false
                })
    ...
    title = {<h5 className = "text-center" >Cashout Earnings</h5>}
    footer={[
              ...
              <>
                <Form>
                    <Form.Item label = "Amount"  >
                        <InputNumber name = "name" value = {values.amount} onChange = {handleChange} max = {state.earnings} min = {0} />
                    </Form.Item>
                </Form>
              </>
          )
            ...
            {failed_notification}
                    ...
                    <Card className = "my-3" title = "Current Earning" >
                        <div className="text-center">
                            <h3>Current earned: {state.earnings} </h3>
                        </div>
                        <div className="p-5">
                                <Button onClick = {showChaoutModal} shape = "round" className = "mx-auto" style ={{width:"100%",height:"50px",borderColor:"green"}} >Cashout</Button>
                        </div>
                    </Card>
